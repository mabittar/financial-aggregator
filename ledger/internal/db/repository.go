...
// UserRepository provides database operations for user accounts.
type UserRepository struct {
	conn *pgxpool.Pool
}

// NewUserRepository creates a user repository backed by a pgxpool connection pool.
func NewUserRepository(conn *pgxpool.Pool) *UserRepository {
	return &UserRepository{conn: conn}
}

// Create inserts a new user record and returns the generated ID.
func (u *UserRepository) Create(ctx context.Context, user *auth.User) error {
	...
	row := u.conn.QueryRow(ctx, query, user.Email, user.PasswordHash, user.DisplayName)
...
// Repository stores all repository types.
type Repository struct {
	conn *pgxpool.Pool
}

// NewRepository creates a repository backed by a pgxpool connection pool.
...
// CreatePortfolio inserts a new portfolio and returns its ID.
func (r *Repository) CreatePortfolio(ctx context.Context, userID, name string) (string, error) {
	...
	if err := r.conn.QueryRow(ctx, query, userID, name).Scan(&id); err != nil {
		return "", err
	}
	return id, nil
}

// ListPortfolios retrieves portfolios for a user, ordered by name.
...
// ListHoldings retrieves holdings for a user, ordered by asset symbol.
...
// ListHoldingTransactions retrieves transactions for a holding, ordered by execution date.
func (r *Repository) ListHoldingTransactions(ctx context.Context, userID, holdingID string) ([]Transaction, error) {
	...
	rows, err := r.conn.Query(ctx, query, holdingID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// FindIdempotencyKey retrieves an idempotency key by its key string.
// Uses Tx from db package for transaction handling.
func (r *Repository) FindIdempotencyKey(ctx context.Context, tx Tx, key string) (*IdempotencyKey, error) {
...
// UpsertIdempotencyKey inserts or updates an idempotency key record.
// Uses Tx from db package for transaction handling.
func (r *Repository) UpsertIdempotencyKey(ctx context.Context, tx Tx, key, userID, payloadHash string, responseMetadata json.RawMessage) error {
	...
	_, err := tx.Exec(ctx, query, key, userID, payloadHash, responseMetadata)
	return err
}

// InsertMonthlyStatement creates a new monthly statement record.
// Uses Tx from db package for transaction handling.
func (r *Repository) InsertMonthlyStatement(ctx context.Context, tx Tx, userID, portfolioName string, referenceDate time.Time, ingestKey string, rawPayload, parsedPayload json.RawMessage, source string) (string, error) {
	...
	var id string
	if err := tx.QueryRow(ctx, query, userID, portfolioName, referenceDate, ingestKey, rawPayload, parsedPayload, source).Scan(&id); err != nil {
		return "", err
	}
	return id, nil
}

// UpdateMonthlyStatement updates an existing monthly statement record.
// Uses Tx from db package for transaction handling.
func (r *Repository) UpdateMonthlyStatement(ctx context.Context, tx Tx, userID, portfolioName string, referenceDate time.Time, ingestKey string, rawPayload, parsedPayload json.RawMessage, source string) (string, error) {
	...
	var id string
	if err := tx.QueryRow(ctx, query, rawPayload, parsedPayload, source, userID, portfolioName, referenceDate, ingestKey).Scan(&id); err != nil {
		return "", err
	}
	return id, nil
}

// ListStatementsForPortfolio retrieves all statements for a user and portfolio, ordered by submission date.
func (r *Repository) ListStatementsForPortfolio(ctx context.Context, userID, portfolioName string, referenceDate time.Time) ([]map[string]any, error) {
	...
	rows, err := r.conn.Query(ctx, query, userID, portfolioName, referenceDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// ConfirmStatements marks a monthly statement as confirmed.
// Uses Tx from db package for transaction handling.
func (r *Repository) ConfirmStatements(ctx context.Context, tx Tx, userID, portfolioName string, referenceDate time.Time) error {
	...
	_, err := tx.Exec(ctx, query, userID, portfolioName, referenceDate)
	return err
}